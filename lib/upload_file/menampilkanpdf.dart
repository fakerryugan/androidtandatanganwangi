      ...
      body: GestureDetector(
        onTapDown: (details) {
          if (waitingForTap && currentQrData != null) {
            ...
            setState(() {
              qrDataList.add(currentQrData!);
              qrPositions.add(localOffset);
              currentQrData = null;
              waitingForTap = false;
            });
          }
        ...
        child: Stack(
          children: [
            SfPdfViewer.file(file, controller: pdfViewerController),
          ...
          FloatingActionButton.extended(
            ...
            onPressed: () async {
              ...
              if (result != null && result['encrypted_link'] != null) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Silakan klik posisi di PDF untuk QR.'),
                  ),
                );
              }
            },
