...
import 'package:syncfusion_flutter_pdf/pdf.dart' as pdf_lib; // Library untuk MENULIS PDF
...
class _PdfViewerPageState extends State<PdfViewerPage> {
  ...
  List<Size> _pdfPageSizes = []; // Menyimpan ukuran asli halaman PDF
    ...
    if (widget.qrData != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        _activeQrNotifier.value = {
          ...widget.qrData!,
          'position': Offset(viewSize.width / 2 - _initialQrSize / 2, viewSize.height / 3),
          'size': _initialQrSize, 'locked': false, 'selected_page': 1,
        };
      });
    }
  }

  // Salin seluruh fungsi ini ke dalam class _PdfViewerPageState
  ...
  Future<void> _saveActiveQrToPdf() async {
    ...
    try {
      ...
      scaffold?.showSnackBar(const SnackBar(content: Text('Menyimpan QR ke PDF...')));

      final originalFileBytes = await File(_currentPdfPath).readAsBytes();
      final pdf_lib.PdfDocument document = pdf_lib.PdfDocument(inputBytes: originalFileBytes);
      ...
      // Kalkulasi aspek rasio (sudah benar)
      ...
      if (pdfPageAspectRatio > viewAspectRatio) {
        renderedImageSize = Size(viewSize.width, viewSize.width / pdfPageAspectRatio);
        imagePadding = Offset(0, (viewSize.height - renderedImageSize.height) / 2);
      } else {
        renderedImageSize = Size(viewSize.height * pdfPageAspectRatio, viewSize.height);
        imagePadding = Offset((viewSize.width - renderedImageSize.width) / 2, 0);
      }

      const double yOffsetCorrection = 40.0;
      final Offset positionOnImage = Offset(
        positionOnScreen.dx - imagePadding.dx,
        (positionOnScreen.dy - imagePadding.dy) + yOffsetCorrection,
      );

      final double scale = pdfPageSize.width / renderedImageSize.width;
      final double pdfX = positionOnImage.dx * scale;
      final double pdfY = positionOnImage.dy * scale;
      final double qrSizeInPdf = sizeOnScreen * scale;

      final Offset topLeft = Offset(pdfX, pdfY);
      // ✅ TYPO SUDAH DIPERBAIKI DI SINI
      final Offset bottomRight = Offset(pdfX + qrSizeInPdf, pdfY + qrSizeInPdf);

      final Offset finalTopLeft = Offset(
          pdfPageSize.width - topLeft.dx,
          pdfPageSize.height - topLeft.dy
      );
      final Offset finalBottomRight = Offset(
          pdfPageSize.width - bottomRight.dx,
          pdfPageSize.height - bottomRight.dy
      );

      final Rect finalRect = Rect.fromPoints(finalTopLeft, finalBottomRight);

      debugPrint("=== MAPPING DENGAN KOREKSI OFFSET Y ===");
      debugPrint("Rect Final yang digambar: $finalRect");

      final imageBytes = await QrPainter(data: signToken, version: QrVersions.auto, gapless: true).toImageData(2048);
      if (imageBytes == null) throw Exception('Gagal membuat gambar QR.');
      final pdf_lib.PdfBitmap pdfImage = pdf_lib.PdfBitmap(imageBytes.buffer.asUint8List());
      ...
      scaffold?.showSnackBar(const SnackBar(content: Text('QR berhasil disimpan!')));
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: _buildAppBar(),
      // ✅ UI menjadi lebih bersih tanpa widget tersembunyi
      body: _isLoadingPdf
          ...
          return Visibility(
            ...
            child: Padding(
              ...
              child: FloatingActionButton.extended(
                ...
                onPressed: () {
                  if (hasActiveQr) { _saveActiveQrToPdf(); }
                  else { _addNewQrCode(); }
                },
                ...
                icon: Icon(hasActiveQr ? Icons.save : Icons.add, color: Colors.black),
              ),
            ),
          );
        },
  ...
  Widget _buildBackButton() => Container(
    ...
    child: IconButton(icon: const Icon(Icons.arrow_back_outlined, color: Color(0xFF172B4C)), onPressed: _showCancelConfirmationDialog),
  );
  ...
  Widget _buildSendButton() => Container(
    ...
    child: _isSending
        ...
        : IconButton(icon: const Icon(Icons.send, color: Colors.white), onPressed: _sendDocument),
  );

  void _addNewQrCode() async {
    if (_activeQrNotifier.value != null) {
      scaffoldMessengerKey.currentState?.showSnackBar(const SnackBar(content: Text('Selesaikan tanda tangan yang ada terlebih dahulu.')));
      return;
    }
    final totalPages = _pageImages.length;
    final result = await showInputDialog(
      context: context, formKey: _formKey, nipController: nipController,
      tujuanController: tujuanController, showTujuan: true, totalPages: totalPages, documentId: widget.documentId,
    );
    if (result != null && result['sign_token'] != null) {
      final viewSize = MediaQuery.of(context).size;
      _activeQrNotifier.value = {
        'sign_token': result['sign_token'], 'selected_page': result['selected_page'],
        'position': Offset(viewSize.width / 2 - _initialQrSize / 2, viewSize.height / 3),
        'size': _initialQrSize, 'locked': false,
      };
  ...
  Future<void> _sendDocument() async {
    try {
      ...
      var request = http.MultipartRequest('POST', Uri.parse('$baseUrl/documents/replace/${widget.documentId}'));
      request.headers['Authorization'] = 'Bearer $authToken';
      request.files.add(await http.MultipartFile.fromPath('pdf', _currentPdfPath, contentType: MediaType('application', 'pdf')));
      var response = await request.send();
      if (response.statusCode == 200) {
        scaffoldMessengerKey.currentState?.showSnackBar(const SnackBar(content: Text('Dokumen berhasil dikirim!')));
        if (mounted) Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const MyBottomNavBar()), (route) => false);
      } else {
        var responseBody = await response.stream.bytesToString();
        throw Exception(jsonDecode(responseBody)['message'] ?? 'Gagal mengirim dokumen');
      }
    } catch (e) {
      scaffoldMessengerKey.currentState?.showSnackBar(SnackBar(content: Text('Error: ${e.toString()}')));
    } finally {
  ...
  Future<void> _showCancelConfirmationDialog() async => await showDialog(
    ...
    actions: [
      ...
      TextButton(
        onPressed: _isProcessing ? null : () async { Navigator.of(context).pop(); await _cancelDocumentRequest(); },
        child: _isProcessing ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Ya, Batalkan'),
      ),
    ],
  ),
  );
  ...
  Future<void> _cancelDocumentRequest() async {
    try {
      ...
      if (response['success'] == true) {
        ...
        scaffoldMessengerKey.currentState?.showSnackBar(SnackBar(content: Text(response['message'] ?? 'Dokumen dibatalkan')));
        if (mounted) Navigator.of(context, rootNavigator: true).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => const MyBottomNavBar()), (route) => false);
      } else {
        scaffoldMessengerKey.currentState?.showSnackBar(SnackBar(content: Text(response['message'] ?? 'Gagal membatalkan')));
      }
    } catch (e) {
      scaffoldMessengerKey.currentState?.showSnackBar(SnackBar(content: Text('Error: ${e.toString()}')));
    } finally {
