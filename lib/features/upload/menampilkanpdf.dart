...
class _PdfViewerPageState extends State<PdfViewerPage> {
  ...
  Future<void> _saveActiveQrToPdf() async {
    ...
    try {
      ...
      // 1. Gambar Kotak Putus-Putus / Solid Biru Tipis
      final pdf_lib.PdfPen pen = pdf_lib.PdfPen(
        pdf_lib.PdfColor(0, 50, 200), // Biru
        width: 1.5,
      );
      pen.dashStyle = pdf_lib.PdfDashStyle.dash; // Putus-putus
      
      graphics.drawRectangle(bounds: finalRect, pen: pen);

      // 2. Tulis Text Indikator
      final pdf_lib.PdfFont font = pdf_lib.PdfStandardFont(
        pdf_lib.PdfFontFamily.helvetica, 
        10, 
        style: pdf_lib.PdfFontStyle.bold
      );

      // Text di tengah kotak
      graphics.drawString(
        "AREA TANDA TANGAN\n(Klik untuk Menandatangani)", 
        font, 
        bounds: finalRect, 
        brush: pdf_lib.PdfSolidBrush(pdf_lib.PdfColor(0, 50, 200)),
        format: pdf_lib.PdfStringFormat(
          alignment: pdf_lib.PdfTextAlignment.center,
          lineAlignment: pdf_lib.PdfVerticalAlignment.middle,
        )
      );
